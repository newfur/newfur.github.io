...
### 🚀 v3.2.x (当前最新)
* **安卓原生体验与版本统一 (v3.2.0)**:
  - 拦截安卓原生返回键（MainActivity `onBackPressed`），支持优先关闭悬浮面板/对话框，并在阅读界面按返回键返回书库（而非退出 App）。
  - 版本号读取机制统一，直接从 `manifest.json` 动态读取，避免硬编码。
  - 打开书籍时，自动跳转到上次的 TTS 朗读进度句，而不仅仅是视觉上的滚动位置。
  - 优化移动端拖拽导入覆盖层，防止多点触摸时闪烁。
