...
*   **修复书库备份还原后阅读时间重复累计的 Bug**：
    - 重构了 `importBook()` 中的阅读统计合并策略：对于同一天（`readingDays`）及同一个小时（`hourlyDist`）的阅读秒数，由原先的「直接相加」改为取两份数据中的「最大值（max）」，避免同一段阅读时间因备份还原被重复计入。
    - `totalTime` 改为从合并后的 `readingDays` 逐日重新汇总计算，确保总时间与每日明细完全一致，杜绝虚报。

---

## 🔄 历史更新 (v2.2.8)

*   **修复 TTS 背景预加载断句与正文 DOM 断句不一致导致高亮错位的 Bug**：
    - 重构了 `_extractSentencesFromHtml()` 函数，不再将 HTML 的 TEXT 节点孤立切分，而是采用与 `prepareContainer()` 相同的 DOM 遍历和分块合并逻辑，支持跨行内元素合并未结束的句子。
    - 确保了背景非同步预加载解析出的句子列表与正式加载渲染 DOM 時的句子列表在數量和內容上 100% 相同，徹底解決了在包含大量格式化短句或對話的章節中高亮顯示與語音內容對應不上的問題。

---

## 🔄 历史更新 (v2.2.7)

*   **修复分栏双页模式在侧边栏/AI面板开启时文字排版错乱、跨越中缝的 Bug**：
    - 在 `applyLayoutDimensions()` 布局计算中，改为根据侧边栏和 AI 面板的实际可见大小动态计算排除后的视窗宽度，防止页面宽度计算过宽导致浏览器分栏计算失效并回退为单栏。
    - 绑定侧边栏切换和 AI 面板开启/关闭事件，在切换时即时重新计算排版，并在 350ms 的 CSS 动画过渡后再次校准。
    - 在拖曳调整 AI 伴侣面板宽度时，实时更新分栏排版，使页面和分栏能平滑、动态地随着拉伸缩放。

---

## 🔄 历史更新 (v2.2.6)

*   **桌面端上下滚动模式默认左右边距设置为 2%**：优化了桌面端上下滚动模式下的默认左右边距为 2% 以充分利用屏幕空间，同时移动端以及左右分页翻页模式保持默认 5% 边距。
