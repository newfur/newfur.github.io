...
export class TTSEngine {
  ...
  _fetchSentence(index) {
    ...
    this._downloadSentenceAudio(sentence).then(blob => {
      ...
      // 增加失敗計數 (任何在這裡捕獲的錯誤都是 WebSocket/網絡加載失敗)
      this.consecutiveWsFailures = (this.consecutiveWsFailures || 0) + 1;
      if (this.consecutiveWsFailures >= 3) {
        console.warn("WebSocket TTS failed 3 times consecutively. Switching to native Web Speech API.");
        ...
        alert("微軟雲端語音連接失敗（可能由於移動端跨域限制或無網絡），已自動為您切換為本地系統語音朗讀。");
