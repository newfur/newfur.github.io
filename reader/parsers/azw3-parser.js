...
export class Azw3Parser {
  ...
  async parse() {
    ...
    // 將龐大的 HTML 切分成多個章節段落
    const chapters = this._splitIntoChapters(fullHtml);
  ...
  _parseRecord0() {
    ...
    if (data.length >= this.mobiHeaderOffset + 16) {
      ...
      if (headerLength >= 228 && mobiVersion >= 5 && data.length >= this.mobiHeaderOffset + 228) {
        ...
        console.log(`Parsed MOBI Header: version=${mobiVersion}, headerLength=${headerLength}, extraDataFlags=0x${this.extraDataFlags.toString(16)}`);
      }
    }
  }
