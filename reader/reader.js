  ...
  return new Promise((resolve) => {
    chrome.storage.local.get(['aiProfiles', 'activeAIProfileId', 'aiProvider', 'aiApiKey', 'aiEndpoint', 'aiModel', 'aiPromptsTemplates'], async (res) => {
      ...
      if (res.aiPromptsTemplates && Array.isArray(res.aiPromptsTemplates)) {
        ...
        // 同步更新默認提示詞：用最新的 locale 值覆蓋 isDefault=true 的項目
        const latestDefaults = getAISuggestions();
        let needsSync = false;
        for (const defaultItem of latestDefaults) {
          const stored = aiPromptsTemplatesList.find(t => t.isDefault && t.key === defaultItem.key);
          if (stored) {
            // 更新 label 和 prompt 為最新翻譯值
            if (stored.prompt !== defaultItem.prompt || stored.label !== defaultItem.label) {
...
function initTTSPanelVoices(filename, isBookOpening = false) {
  ...
  if (ttsDefaultVoice && !hasDefaultVoice && (tts.ttsProvider === 'openai' || tts.ttsProvider === 'local')) {
    ...
    // Dynamically register in the engine as well
    tts.setVoice(ttsDefaultVoice);
  }

  // 優先使用保存的語音，其次保留當前已選中的語音，最後是默認優先語音
  const savedVoice = currentBook?.progress?.ttsVoice;
  const allAvailableVoices = select.querySelectorAll('option');
  const availableVoiceValues = Array.from(allAvailableVoices).map(opt => opt.value);
...
function getVisNodeOptions(node, isDark) {
  ...
  if (node.level === 0) {
    return {
      ...
      color: {
        background: isDark ? '#007aff' : '#0055d6',
        border: isDark ? '#007aff' : '#0055d6',
        highlight: {
      ...
      shadow: {
        enabled: true,
        color: isDark ? 'rgba(0,122,255,0.25)' : 'rgba(0,122,255,0.15)',
        size: 8,
        x: 0,
        y: 4
      }
    ...
    return {
      ...
      font: {
        size: 14,
        color: isDark ? '#ffffff' : branchColor,
        bold: true,
        face: 'system-ui, -apple-system, sans-serif'
      },
      color: {
        background: hexToRgba(branchColor, isDark ? 0.2 : 0.06),
        border: branchColor,
        highlight: {
          background: hexToRgba(branchColor, isDark ? 0.3 : 0.12),
          border: branchColor
        }
      },
      borderWidth: 2.5,
      shadow: {
        enabled: true,
        color: hexToRgba(branchColor, isDark ? 0.2 : 0.1),
        size: 6,
        x: 0,
        y: 3
      }
    ...
    return {
      ...
      color: {
        background: isDark ? '#242426' : '#ffffff',
        border: hexToRgba(branchColor, 0.45),
        highlight: {
      ...
      shadow: {
        enabled: true,
        color: isDark ? 'rgba(0,0,0,0.25)' : 'rgba(0,0,0,0.05)',
        size: 4,
        x: 0,
        y: 2
      }
      ...
      if (code.startsWith('mindmap') || code.includes('\nmindmap')) {
        ...
        try {
          ...
          const visEdges = edges.map(e => {
            ...
            // Width scales down with depth: Level 1 (from root) is thickest, Level 2 is medium, Level 3+ is thin
            let width = 1.2;
            if (targetNode) {
              if (targetNode.level === 1) width = 3.5;
              else if (targetNode.level === 2) width = 2.2;
            }
            ...
            return {
              ...
              to: e.to,
              color: {
                color: hexToRgba(branchColor, 0.8), // Slightly transparent line for elegance
          ...
          const options = {
            ...
            physics: {
              ...
              forceAtlas2Based: {
                gravitationalConstant: -220, // Increased repulsion to spread nodes out
                centralGravity: 0.01,         // Slight gravity to keep it grouped
                springLength: 130,            // Longer spring length for more spacing
                springConstant: 0.05,
                avoidOverlap: 1.0
              },
              stabilization: {
