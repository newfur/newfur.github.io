...
function clearSelectionChangeTimer() {
  if (!selectionChangeTimeout) return;
...
function captureReaderContext() {
  ...
  return {
    ...
    book,
    bookId: book?.id,
    isCurrent: () => Boolean(book && readerOperations.isCurrent(operation, book.id) && currentBook === book)
  };
}
...
function initUIEventBindings() {
  ...
  if (clearAIHistoryBtn) {
    clearAIHistoryBtn.addEventListener('click', async () => {
      ...
      if (confirm(confirmMsg)) {
        try {
          const context = captureReaderContext();
          const updatedChats = await library.clearAllAIChats(context.bookId);
          if (!context.isCurrent()) return;
          context.book.aiChats = updatedChats;
          renderAIChatHistory();
        } catch (err) {
  ...
  function startTTSProgressHeartbeat() {
    ...
    const operation = readerOperations.currentToken();
    const ownerBookId = tts.ownerBookId;
    const sessionId = tts.playbackGeneration;
    ttsProgressHeartbeatTimer = setInterval(() => {
      if (tts.isPlaying && !tts.isPaused) {
        saveTTSProgressImmediately(operation, sessionId, ownerBookId);
      }
    }, 5000);
  }
  ...
  tts.onStateChange = () => {
    ...
    if (!tts.isPlaying || tts.isPaused) {
      saveTTSProgressImmediately(readerOperations.currentToken(), tts.playbackGeneration, tts.ownerBookId);
      stopTTSProgressHeartbeat();
    } else {
      // 播放中：啟動心跳保存
  ...
  tts.onSentenceStart = (index, sessionId, ownerBookId) => {
    const operation = readerOperations.currentToken();
    const ownedBook = currentBook;
    const isCurrentTTSOwner = () =>
      ownerBookId === ownedBook?.id &&
      readerOperations.isCurrent(operation, ownerBookId) &&
      currentBook === ownedBook &&
      tts._isCurrentSession(sessionId, ownerBookId);
    if (!isCurrentTTSOwner()) return;
    // 同步播放按鈕狀態
    updatePlayPauseButtonIcon();
    ...
    if (isCurrentTTSOwner()) {
      ...
      if (sentence) {
        ...
        library.updateProgress(ownerBookId, progressUpdate).catch(e => console.warn('[TTS] Progress save failed:', e));
        if (isCurrentTTSOwner()) ownedBook.progress = { ...ownedBook.progress, ...progressUpdate };
      }
    }

    // 在翻頁模式下，如果朗讀的句子不在當前可見頁面上，則自動翻頁
    if (isCurrentTTSOwner() && document.body.classList.contains('layout-paginated')) {
  ...
  document.addEventListener('selectionchange', () => {
    ...
    if (readerView && readerView.classList.contains('view-active')) {
      clearSelectionChangeTimer();
      const selectionOperation = readerOperations.currentToken();
      const selectionBook = currentBook;
      const selection = window.getSelection();
      const selectionSnapshot = selection?.rangeCount ? {
        rangeCount: 1,
        text: selection.toString(),
        range: selection.getRangeAt(0).cloneRange()
      } : { rangeCount: 0, text: '', range: null };
      selectionChangeTimeout = setTimeout(ownedCallback(
        () => readerOperations.isCurrent(selectionOperation, selectionBook?.id) && currentBook === selectionBook,
        () => {
          selectionChangeTimeout = null;
          handleTextSelection(null, selectionSnapshot);
        }
      ), 250);
    }
  });
...
function startReadingTracker(bookId) {
  stopReadingTracker();
  const tracker = readingTrackerLease.acquire(bookId);

  lastReadingHeartbeat = Date.now();
  lastUserActivityTime = Date.now();

  readingSessionTimer = setInterval(async () => {
    if (!readingTrackerLease.isCurrent(tracker) || currentBook?.id !== bookId) {
      stopReadingTracker(tracker);
      return;
    }
    ...
    if (readingTrackerLease.isCurrent(tracker)) lastReadingHeartbeat = now;
  }, 10000);
  return tracker;
}

function stopReadingTracker(tracker = readingTrackerLease.current()) {
  if (!tracker || !readingTrackerLease.release(tracker)) return false;
  if (readingSessionTimer) clearInterval(readingSessionTimer);
  readingSessionTimer = null;
  return true;
}

async function saveReadingTime(bookId = currentBook?.id, tracker = readingTrackerLease.current()) {
  if (bookId) {
    const now = Date.now();
    const isUserActive = isReadingTimeActive(now);
    if (isUserActive) {
...
async function closeCurrentBook(triggerBack = true) {
  ...
  await saveReadingTime(closingBookId, closingTracker);
  if (!readerOperations.isCurrent(operation, closingBookId)) return;
...
function saveTTSProgressImmediately(operation = readerOperations.currentToken(), sessionId = tts.playbackGeneration, ownerBookId = tts.ownerBookId) {
  const book = currentBook;
  const isCurrentTTSOwner = () =>
    ownerBookId === book?.id &&
    readerOperations.isCurrent(operation, ownerBookId) &&
  ...
  if (isCurrentTTSOwner()) {
    ...
    library.updateProgress(ownerBookId, progressUpdate).catch(e => console.warn('[TTS] Immediate save failed:', e));
    if (isCurrentTTSOwner()) book.progress = { ...book.progress, ...progressUpdate };
  }
}
...
function initTTSPanelVoices(filename, isBookOpening = false) {
  ...
  const getLangDisplayName = (code) => {
    ...
    if (code.startsWith('en')) return getMsg('tts_lang_en') || 'English';
    if (code.startsWith('ja')) return getMsg('tts_lang_ja') || '日本語';
    if (code.startsWith('ko')) return getMsg('tts_lang_ko') || '한국어';
  ...
  if (matchedVoices.length > 0) {
    ...
    groupRec.label = `🌟 ${getMsg('recommended') || 'Recommended'} (${getLangDisplayName(lang)})`;
    matchedVoices.forEach(voice => {
      groupRec.appendChild(createOption(voice));
  ...
  if (otherVoices.length > 0) {
    ...
    groupOther.label = `🌐 ${getMsg('other_voices') || 'All Other Voices'}`;
    otherVoices.forEach(voice => {
      groupOther.appendChild(createOption(voice));
      if (voice.name === ttsDefaultVoice) hasDefaultVoice = true;
    });
...
// 監聽文字選取
function handleTextSelection(e, snapshot = null) {
  const selection = snapshot || window.getSelection();
  ...
  const selectedText = (snapshot ? snapshot.text : selection.toString()).trim();
  const menu = document.getElementById('selection-menu');

  if (selectedText.length > 0) {
    selectedTextState = selectedText;
    selectedTextRange = snapshot ? snapshot.range : selection.getRangeAt(0).cloneRange();
...
async function handleAddHighlight(color) {
  ...
  const updatedNotes = await library.saveNote(context.bookId, note);
  if (!context.isCurrent()) return;
  context.book.notes = updatedNotes;
...
async function handleAddBookmark() {
  ...
  await library.saveBookmark(context.bookId, bookmark);
  if (!context.isCurrent()) return;
  renderHighlightsList();
}
...
async function renderHighlightsList() {
  ...
  const book = await library.getBook(context.bookId);
  if (!book || !context.isCurrent()) return;
...
async function deleteBookmarkHandler(bookmarkId) {
  if (currentBook) {
    const context = captureReaderContext();
    await library.deleteBookmark(context.bookId, bookmarkId);
    if (!context.isCurrent()) return;
    await renderHighlightsList();
  }
}
...
async function deleteNoteHandler(noteId) {
  if (currentBook) {
    ...
    const updatedNotes = await library.deleteNote(context.bookId, noteId);
    if (!context.isCurrent()) return;
    context.book.notes = updatedNotes;
...
function addDeleteButtonToGroup(groupEl, chat) {
  ...
  deleteBtn.addEventListener('click', async (e) => {
    ...
    if (confirm(confirmMsg)) {
      try {
        const context = captureReaderContext();
        const updatedChats = await library.deleteAIChat(context.bookId, chat.chatId);
        if (!context.isCurrent()) return;
        context.book.aiChats = updatedChats;
