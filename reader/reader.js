...
async function sendCustomAIQuery() {
  ...
  try {
    ...
    // 判斷是否為針對「本章」、「本頁」或需要局部章節脈絡的特定提問（包括總結、思維導圖等）
    const isChapterOrPageQuery = /本章|当前章|这一章|这章|该章|这个章节|当前章节|本章节|这一节|这节|本节|当前节|本页|当前页|这一页|这页|当前页面|该页|总结|大纲|思维导图|脑图|核心观点|核心要点|结构|提纲|脉络/.test(query);
    
    const shouldDoFullBookSearch = bookChunksCache.length > 0 && (isFullBookQuery || !isChapterOrPageQuery);
    
    if (shouldDoFullBookSearch) {
      // 進行全書檢索
      ...
      if (relevantChunks.length > 0) {
        ...
        isFullBookContext = true;
      }
    }
    
    if (!bookContext && bookContentEl) {
      // 退回局部章節上下文，克隆並清理非文字標籤以防樣式表 CSS 污染上下文
      const clone = bookContentEl.cloneNode(true);
      clone.querySelectorAll('script, style, noscript, iframe').forEach(el => el.remove());
      bookContext = clone.textContent.trim().substring(0, 25000);
    }
