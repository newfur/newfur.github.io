...
[![Version](https://img.shields.io/badge/version-3.4.70-orange)](https://github.com/newfur/newfur.github.io/releases/tag/v3.4.70)
...
針對跨平台行動閱讀、長時間沉浸聽書與大容量圖書儲存，全新版本建構了先進的底層核心：
...
- **Route B（行動端原生硬體雙緩衝無縫引擎 - Native Twin-Player Engine）**：
  - **0ms 極速無縫換句 (Gapless Playback)**：iOS (`AVAudioPlayer`) 與 Android (`MediaPlayer`) 原生層均實現了**雙底層硬體通道乒乓交叉緩衝（Ping-Pong Buffering）**。當當前句子在通道 A 發聲時，下一句音訊已在背景完成合成並在通道 B 完成物理硬體預熱（`prepareToPlay`）；當前句播放結束瞬間，硬體通道毫秒級無感切換，徹底消滅句間延遲與空隙！
  - **全鏈路主執行緒 RunLoop 排程**：iOS 端原生層所有播放器生命週期、事件監聽與狀態管理嚴格綁定於主執行緒 `RunLoop.main`，確保蘋果 CoreAudio 底層硬體事件與代理回呼即時精準分發。
...
### 2. 🗄️ 徹底解耦的輕量高效能儲存架構 (Decoupled Storage Engine)
- **實體與元數據分表獨立儲存**：書籍正文二進位、閱讀進度、螢光標記與檢索索引在 IndexedDB 中實行獨立分表儲存，杜絕大物件巢狀與冗餘序列化，支援大部頭書籍秒級存取。
- **儲存高效與健康可控**：採用輕量緊湊的沙盒資料管理，即便匯入數百本大容量圖文書籍，應用自身體積與執行快取也始終保持輕盈，保障極佳的讀寫效能與裝置空間健康。

### 3. 🖼️ 全域 512px 動態封面智慧最佳化與跨端廣播 (`compressCoverImage`)
- **智慧 Canvas 降採樣管線**：解析圖書瞬間自動將高解析度封面等比最佳化至 512×512 視網膜規格（體積壓縮至僅 20KB~40KB），既保證了視覺細膩度，又大幅降低了記憶體與跨程序橋接通訊開銷。
- **鎖定畫面與通知列秒級呈現**：最佳化後的輕量封面在行動裝置鎖定畫面媒體控制中心與系統通知列實現無感毫秒級即時展示。
